...
function validateOrderUser(data) {
  ...
  if (!data.name.trim()) errors.push("姓名不可為空");
  if (!/^09\d{8}$/.test(data.tel)) errors.push("電話格式不正確");
  if (!data.email.includes("@")) errors.push("Email 格式不正確");
  if (!data.address.trim()) errors.push("地址不可為空");
